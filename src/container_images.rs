        ...
        container.data.into_iter().for_each(|os_data| {
            ...
            match os {
                Some(os) => os.releases.extend(os_data.images.into_iter().flat_map(|image| {
                    ...
                    let shared_dirs = container.shared_dirs.clone();
                ...
                None => log::error!("OS {} not found in distros", os_data.os_name),
            }
        })
    });
}
...
async fn fetch_container_images() -> Option<Vec<DockerData>> {
    ...
    tags.iter()
        .flat_map(|tag| DockerData::try_from(&**tag).inspect_err(|e| log::error!("Failed to parse metadata from tag {tag}: {e}")))
        .collect::<Vec<_>>()
        .into()
}
...
impl<'a> TryFrom<Tag<'a>> for DockerData {
    ...
    fn try_from(tag: Tag<'a>) -> Result<Self, Self::Error> {
        ...
        let images: Metadata = serde_json::from_str(&data).map_err(|e| e.to_string())?;
        Ok(DockerData {
            url: format!("{GH_CONTAINER_SOURCE}:{tag}"),
            privileged: images.privileged,
            shared_dirs: images.shared_dirs,
            data: images.os_data,
        })
    }
}
